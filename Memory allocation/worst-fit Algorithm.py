    ...
    for i in range(n):
        ...
        for j in range(m):
            if blockSize[j]>=processSize[i]:
                ...
                if best==-1 or blockSize[j]>blockSize[best]: #opposite of best-fit,we allocate to the largest available block to the job
                    best=j 
        if best != -1:
            allocated[i]=best
...
if __name__ == '__main__':
    ...
    allocation_result ,blockSize= worst_fit(blockSize,processSize)
